...
- 说明：
  ...
  - `cover_url` 为可选字段；未传时应返回 `null`（可先上传图片拿到 URL 再写入该字段）
...
- 说明：
  ...
  - 请求体中的 `list_price` 表示目录价（分）

## 管理员更新票种信息

- URL: `/exhibition/:eid/tickets/:tid`
- Method: `PATCH`
- Request Header:
  ```ts
  { Authorization: `Bearer ${token}` }
...
- Request Body:
  ```ts
  Exhibition.TicketCategoryPatch
  ```
- Response Body:
  ```ts
  Exhibition.TicketCategory
...
- Response Status:
  - `200 OK`：更新成功
  - `400 Bad Request`：未提供任何可更新字段，或字段值不合法
  - `401 Unauthorized`：未认证
  - `403 Forbidden`：无管理员权限
  - `404 Not Found`：票种不存在或不属于该展会
- 说明：
  - 可更新字段范围以 `Exhibition.TicketCategoryPatch` 为准，包含 `name`、`valid_duration_days`、`refund_policy`、`admittance`、`list_price`
  - `eid` 与 `tid` 共同定位待更新的票种
  - 未出现在 body 中的字段保持原值不变
  - `list_price`（目录价）在该接口维护
  - 场次销售价不在该接口维护，需走场次价格日历接口
