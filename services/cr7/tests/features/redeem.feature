Feature: 用户已购票的查询与核销
  ...
  Scenario: 一个完成支付的订单拥有一个核销码
    Given 用户 "Alice" 预订 2 张该展会的 "今天" 场次的 "early_bird"
     ...
     Then 订单详情中包含一个核销码
      ...
      And 核销码的准入人数为 "2"
      And 核销码关联订单的场次日期为 "今天"
      And 核销码关联订单的来源为 "DIRECT"
      And 核销码的有效期为场次当天

  Scenario: 用户可以分页查询自己的核销码列表并按状态筛选
    Given 用户已完成 2 个订单支付用于核销码列表查询
     When 用户 "Alice" 查询自己的核销码列表，第 1 页，每页 1 条
     Then 核销码列表总数为 2
      And 核销码列表当前页为 1
      And 核销码列表每页数量为 1
      And 核销码列表返回 1 条记录
    Given 运营人员核销用户 "Alice" 的第 1 个订单核销码
     When 用户 "Alice" 按状态 "REDEEMED" 查询自己的核销码列表，第 1 页，每页 10 条
     Then 按状态筛选的核销码列表总数为 1
      And 按状态筛选的核销码列表返回 1 条记录
      And 按状态筛选结果中的订单 ID 与第 1 个订单一致
      And 按状态筛选结果中的订单场次日期为 "今天"
  ...
  Scenario: 未生效订单的核销码不可用
    Given 用户 "Alice" 预订 1 张该展会的 "1天后" 场次的 "early_bird"
     When 用户 "Alice" 完成支付
     Then 用户 "Alice" 有一个有效的核销码
     When 运营人员将用户 "Alice" 的订单核销码扫码核销
     Then 操作失败，状态码为 409
  ...
  Scenario: 已经核销的订单不能发起退款
    ...
    Given "成人票" 库存为 2
    Given 用户 "Alice" 预订 1 张该展会的 "今天" 场次的 "成人票"
     When 用户 "Alice" 完成支付
     Then 用户 "Alice" 有一个有效的核销码
    Given 订单已被核销
     When 用户发起退款请求
  ...
  Scenario: 已经处于退款流程的订单不能被核销
    Given 该展会追加了一个 "成人票" 票种, 退票策略是 "截止时间为场次开始前 48 小时"
    Given "成人票" 库存为 2
    Given 用户 "Alice" 预订 1 张该展会的 "3天后" 场次的 "成人票"
     When 用户 "Alice" 完成支付
     Then 用户 "Alice" 有一个有效的核销码
  ...
  # Scenario: 转移核销码
  #   Given 用户 "Bob" 预订 1 张该展会的 "今天" 场次的 "early_bird"
  #    When 用户 "Bob" 完成支付
  #    Then 用户 "Bob" 有一个有效的核销码

  #    When 用户 "Bob" 查询自己的核销码列表，第 1 页，每页 1 条
  #    Then 用户 "Bob" 核销码列表总数为 1

  #    When 用户 "Alice" 在三方票同步页面输入用户 "Bob" 的核销码并提交转移请求
  #    Then 转移成功
  #     And 核销码的转移记录中包含从 "Bob" 到 "Alice" 的转移事件

  #   When 用户 "Bob" 查看自己的核销码列表，第 1 页，每页 1 条
  #   Then 用户 "Bob" 核销码列表总数为 0

  #   When 用户 "Alice" 查看自己的核销码列表，第 1 页，每页 1 条
  #   Then 用户 "Alice" 核销码列表总数为 1
  #    And 核销码列表返回的核销码与用户 "Bob" 的核销码一致
