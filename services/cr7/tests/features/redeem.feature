Feature: 用户已购票的查询与核销
  ...
  Scenario: 用户可以分页查询自己的核销码列表并按状态筛选
    Given 用户已完成 2 个订单支付用于核销码列表查询
     When 用户 "Alice" 第 1 次查询自己的核销码列表，第 1 页，每页 1 条
     Then 核销码列表总数为 2
      And 核销码列表当前页为 1
      And 核销码列表每页数量为 1
      And 核销码列表返回 1 条记录
  ...
  Scenario: 转移核销码
    Given 用户 "Bob" 预订 1 张该展会的 "今天" 场次的 "early_bird"
     ...
     When 用户 "Bob" 第 1 次查询自己的核销码列表，第 1 页，每页 1 条
     ...
     When 管理员查看该核销码的转移记录
     Then 核销码的转移记录中有一条记录，转出用户为 "Bob"，转入用户为 "Alice"

     When 用户 "Bob" 第 2 次查询自己的核销码列表，第 1 页，每页 1 条
