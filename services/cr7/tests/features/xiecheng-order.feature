Feature: 对接携程 OTA 订单系统
  ...
  Background:
    ...
    Given 展会添加票种 "早鸟票", 准入人数为 1, 有效期为场次当天, 价格为 100 元
      And "早鸟票" 库存为 2
    Given 展会添加票种 "单人票", 准入人数为 1, 有效期为场次当天, 价格为 200 元
      And "单人票" 库存为 2

    Given 用户在携程上创建了一个订单
      And 携程订单的包含 "早鸟票" 1 张，场次时间为 "今天", 价格是 10000 分
      And 携程订单的总价为 "早鸟票" 的价格, 即 10000 分
  ...
  Scenario: 管理员可以查看单条携程订单同步记录
    ...
    Then 管理员在系统后台可以获取订单号 "xc_order_12345" 的携程同步记录
     ...
     And 同步记录中包含订单总价 "早鸟票" 的价格, 即 10000 分
  ...
  Scenario: 用户在携程下单后，完成支付后又取消了订单
   ...
   Given 携程 service name 是 "QueryOrder" 的订单查询请求
    ...
    Then 再次退款后管理员查看场次 "今天" 的 "早鸟票" 库存应该是 2
