Feature: 对接携程 OTA 订单系统
  ...
  Background:
    ...
    Given 展会添加票种 "早鸟票", 准入人数为 1, 有效期为场次当天
      And "早鸟票" 库存为 2
      And 管理员在 "早鸟票" 上添加携程编号 "xc_early_bird"
    Given 展会添加票种 "单人票", 准入人数为 1, 有效期为场次当天
      And "单人票" 库存为 2
      And 管理员在 "单人票" 上添加携程编号 "xc_single_ticket"
  ...
  Scenario: 管理员可以看到携程订单记录
    When 用户提交订单
    Then cr7 系统收到订单创建通知
     And 订单信息可以正常解密
    Then 管理员在系统后台可以看到 1 条携程订单记录
     And 订单记录内容包含订单号 "xc_order_12345"，序列号 "xc_seq_12345", 同步状态为 "成功"
     And 订单记录中包含手机号 "13800138000"，国别码 "+86"
     And 订单记录中包含 "早鸟票" 1 张，场次时间为 "今天"
     And 订单记录中包含订单总价 "早鸟票" 的价格
    When 携程再次发送订单创建通知
     And 携程订单号是 "xc_order_12345"
     And 携程 sequence id 是 "xc_seq_54321"
    Then 管理员在系统后台可以看到 2 条携程订单记录
     And 订单记录内容包含订单号 "xc_order_12345",序列号 "xc_seq_54321", 同步状态为 "重复订单"
