Feature: 为订单开发票
  ...
  Background:
    ...
    Given 默认核销展览活动已创建，开始时间为 "今天"，结束时间为 "5天后"
    Given 展会添加票种 "单人票", 准入人数为 1, 有效期为场次当天, 价格是 106 元
      And 票种 "单人票" 库存为 2
  ...
  Scenario: 用户成功申请发票
    ...
    Then 发票服务接收到发票开具请求, 可以正常解密出发票申请信息
     ...
     And 请求中价税合计是 212 元，合计金额是 200 元，合计税额是 12 元
     And 请求中有 1 个发票行项目
     And 发票行项目的第 1 行的发票行性质是正常行，值为 "0"
     And 发票行项目的第 1 行的商品编码是 "3070301000000000000"
     And 发票行项目的第 1 行的项目名称是 "单人票", 数量是 2，单价是 106 元
     And 发票行项目的第 1 行的税率是 6%， 税额是 12 元
  ...
  Scenario: 管理员为用户订单申请发票
    Given 用户预订 2 张该展会的 "今天" 场次的 "单人票"
     ...
     When 管理员申请该订单的发票，发票抬头为 "测试公司"，税号为 "123456789", 邮箱为 "send_me_invoice@example.com"
     Then 发票服务接收到发票开具请求, 可以正常解密出发票申请信息
  ...
  Scenario: 用户订单退款后申请发票
    Given 用户预订 2 张该展会的 "3天后" 场次的 "单人票"
    ...
    When 用户申请该订单的发票，发票抬头为 "测试公司"，税号为 "123456789", 邮箱为 "send_me_invoice@example.com"
    Then cr7 返回错误，提示订单已退款，无法申请发票

    When 用户查看发票申请列表
    Then 发票申请列表有 0 条记录

  # Scenario: 用户申请发票后发票平台开具失败
  #   When 用户发起并完成微信支付
  #   Then 订单状态为已支付
