Feature: Order ticket
  ...
  Scenario: 用户预订多个票种
   Given 该展览已追加票种 "儿童票"
     And "儿童票" 所有场次库存为 3
    When 用户预订 1 张该展会的 "3天后" 场次的 "成人票" 和 2 张 "儿童票"
    Then 预订成功
     And 场次 "3天后" 的 "成人票" 库存为 2
  ...
  Scenario: 预订超过库存数量的门票
    When 用户预订 4 张该展会的 "3天后" 场次的 "成人票"
    Then 预订失败，提示库存不足
     And 场次 "3天后" 的 "成人票" 库存为 3

  Scenario: 预订已过期场次的门票
    When 用户预订 1 张该展会的 "1天前" 场次的 "成人票"
    Then 创建失败，提示场次已过期
