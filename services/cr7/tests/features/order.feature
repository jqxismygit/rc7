Feature: Order ticket
  ...
  Scenario: 预订已过期场次的门票
    Given 展览活动 "艺术展" 已创建，包含场次 "2025-07-01" 和票种 "成人票"
    And 场次 "2025-07-01" 的 "成人票" 库存初始为 2
    When 用户预订 1 张 "艺术展" 的 "2025-07-01" 场次的 "成人票"
    Then 创建失败，提示场次已过期
      And 场次 "2025-07-01" 的 "成人票" 库存为 2
  ...
  Scenario: 管理员可以查看所有订单列表
    Given 展览活动 "艺术展" 已创建，包含场次 "2026-07-01" 和票种 "成人票"
    And 场次 "2026-07-01" 的 "成人票" 库存初始为 10
    And 用户 "Bob" 已成功预订 1 张 "艺术展" 的 "2026-07-01" 场次的 "成人票"
    When 管理员查看订单列表
    Then 返回订单列表成功
