Feature: 微信支付订单
  ...
  Scenario: 过期的订单不能发起支付
    Given 用户预订了 1 张 "CR7" 展会 的 "1天后" 场次的 "成人票"
    Given 订单已过期
    When 用户尝试发起支付
  ...
  Scenario: 已经支付的订单发起退款
    Given 用户预订了 2 张 "CR7" 展会 的 "3天后" 场次的 "成人票"
      And "成人票" 的退票策略是 "允许退票，退票截止时间为场次开始前 48 小时"

    Given 用户已完成支付
     When 用户发起退款请求, 退款原因 "不想去了"
     Then 订单状态更新为  "退款已受理"
      And 订单有退款记录 ID
  ...
  Scenario: 管理员可以看到所有的退款记录
    ...
    Given 用户已完成支付
    Given 用户已发起退款请求，退款原因是 "不想去了"，订单状态为 "退款已受理"
  ...
  Scenario: 不能退票的订单发起退款流程，应该被拒绝
    Given 用户预订了 1 张 "CR7" 展会 的 "3天后" 场次的 "早鸟票"
  ...
  Scenario: 订单中包含允许退票和不允许退票的票种，发起退款请求应该被拒绝
    Given 用户预订了 1 张 "CR7" 展会 的 "3天后" 场次的 "早鸟票" 和 1 张 "成人票"
      And "早鸟票" 的退票策略是 "不允许退票"
      And "成人票" 的退票策略是 "允许退票，退票截止时间为场次开始前 48 小时"
    Given 用户已完成支付
     When 用户发起退款请求
